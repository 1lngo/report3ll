...
_USE_LOCAL = os.path.exists(_WEIGHTS_DIR)
...
# 尝试导入轻量级模型作为备选
try:
    from lightweight_predictor import lightweight_forecast, LIGHTWEIGHT_AVAILABLE
    if not KRONOS_AVAILABLE:
        KRONOS_MODE = "lightweight"
        KRONOS_AVAILABLE = True  # 轻量级模型可用
except ImportError:
...
def kronos_forecast(
    ...
    """
    # 云端环境：使用轻量级模型
    if KRONOS_MODE == "lightweight" and LIGHTWEIGHT_AVAILABLE:
        print("Using lightweight model for cloud deployment")
        ...
        result["model_info"] = f"Lightweight (Cloud Mode) - {result['model_info']}"
        return result
